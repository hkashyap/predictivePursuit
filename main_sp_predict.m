% Main script to run the predictive smooth pursuit experiment
...
if target == "sine"
    nsecs = 25; % Experiment time in seconds
    blankStart = [15]; % start time(s) of blank period(s) separated by comma(s)
    blankEnd = [20];   % end time(s) of blank period(s) separated by comma(s)
    startStim = 0;     % Simulation start time
    nsecs = nsecs/step;
    gain = 1;          % leaky-integrator gain
    alpha = 100;       % P matrix initialized to 1/alpha
    freq = 0.5;        % Frequency of the sinusoidal target
